...
sc.pl.umap(ab1, color = ["hums"], legend_loc = None, title = "UMAP of all Renca samples", save = "renca.png")

sc.set_figure_params(fontsize = 20, figsize = (6,6), dpi_save = 600)
sc.pl.umap(ab1, color = ["nCount_RNA", "nFeature_RNA", "percent.mt"], 
           title = ["RNA counts per cell", "Number of RNA features per cell", "% Mitochondrial genes per cell"], wspace = 0.3, save = "renca_cellcomp.png")


sc.set_figure_params(fontsize = 10, dpi_save = 600)
ab1.obs["hums"] = ab1.obs["seurat_clusters"].astype("string")
sc.pl.umap(ab1, color = ["hums"], legend_loc = "on data", title = "UMAP of all Renca samples", save = "rencacluster.png")


sc.set_figure_params(fontsize = 15, dpi_save = 600)
ab1.obs["hums"] = ab1.obs["sample"].astype("category")
sc.pl.umap(ab1, color = ["hums"], 
           title = "UMAP of all Renca samples", 
           size=10, alpha = 0.5, palette = "Dark2", sort_order=False, save = "rencasample.png")
...
ax.set_title('Proportion of Cell Types by Sample (Renca)', fontsize = "x-large")
ax.grid(False)
ax.tick_params(axis='y', which='major', labelsize=15)
plt.tight_layout()
plt.savefig("figures/renca_cell_proportions.png")
